...
<div class="card border-0 shadow-sm">
    <div class="card-header align-items-center">
        <div class="row">
            ...
            <div class="col-auto">
                <div class="form-row">
                    <div class="col">
                        @include('chats.cloude.partials.menu')
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card-body">
        <form action="{{ request()->is('admin/*') ? route('admin.chats.cloude.edit', $chat->id) : route('chats.cloude.edit', $chat->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select name="model" id="i-openai-model" disabled class="custom-select{{ $errors->has('openai_model') ? ' is-invalid' : '' }}">
                    @foreach(['claude 3'] as $key)
                        <option value="{{ $key }}" @if($key==old('name') or $key==$chat->model) selected @endif>{{ $key }}</option>
                    @endforeach
                </select>
            </div>
