...
@include('shared.breadcrumbs', ['breadcrumbs' => [
    ...
    ['url' => request()->is('admin/*') ? route('admin.chats.gpt') : route('chats.gpt'), 'title' => __('Chats')],
    ['title' => __('Chat')],
]])
    ...
    <div class="card-header">
        <div class="row flex-fill">
            ...
            <div class="col-auto">
                <div class="form-row">
                    <div class="col">
                        <form method="GET" action="{{ route('chats.gpt.show', ['id' => $chat->id]) }}">
                                    ...
                                    <div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow width-64 p-0" id="search-filters">
                                        <div class="dropdown-header py-3">
                                            <div class="row">
                                                ...
                                                <div class="col-auto">
                                                    <a href="{{ route('chats.gpt.show', ['id' => $chat->id]) }}" class="text-secondary">{{ __('Reset') }}</a>
                                                </div>
                                            </div>
                                        </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="dialog">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-footer">
                ...
                @can('dataExport', ['App\Models\User'])
                    <a href="{{ route('chats.gpt.export', ['id' => $chat->id] + Request::query()) }}" target="_self" class="btn btn-primary" id="exportButton">{{ __('Export') }}</a>
                @endcan
            </div>
        </div>
    </div>
